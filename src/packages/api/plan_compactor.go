...
const (
	...
	compactSystemPrompt = "You compress the older part of a trip-planning conversation into a factual state summary so the planning agent can continue seamlessly. " +
		"Call record_summary once. Preserve, when present: travel dates (exact YYYY-MM-DD) and how flexible they are; origin and destination cities and their order; the stated travel mode (driving, train, bus, flying, ferry); the travelers (count, names, relationships, who they usually travel with); flights discussed and which one was CHOSEN (airline, price, times); ferries, trains, or accommodation chosen; budget level and pace; dietary, accessibility, and interest constraints; whether they work while traveling and any stated work-day constraints (e.g. hours to keep free, wifi needs); any fitness routine they keep on the road (gym access, running) and how demanding they want outdoor days to be; places already agreed into the itinerary and places explicitly rejected (with the reason); whether an itinerary was already created or saved; and open questions or decisions still pending. " +
